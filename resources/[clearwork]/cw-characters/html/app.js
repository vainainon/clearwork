...
function renderCharacters() {
  ...
  currentCharacters.forEach((character) => {
    ...
    if (isDead) {
      ...
      actionsHtml = `
        <div class="card-actions single-action">
          <button class="cancel-delete-btn" type="button" ${status.cancelAvailable ? '' : 'disabled'}>Отменить удаление</button>
        </div>
      `;
    } else if (isCurrent) {
      actionsHtml = `
        <div class="card-actions">
          <button class="selected-btn" type="button" disabled>Выбран</button>
      ...
      actionsHtml = `
        <div class="card-actions">
          <button class="${selectClass}" type="button">${selectLabel}</button>
          <button class="delete-btn" type="button" ${canRequestDelete ? '' : 'disabled'}>Удалить</button>
        </div>
      `;
    }
